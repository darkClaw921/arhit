...
- **Claude-интеграция** — при `arhit init` создаётся секция в `CLAUDE.md` с инструкциями для агента
...
```bash
arhit init                          # Инициализация (автоопределение языка)
...
| Команда | Описание |
|---------|----------|
| `arhit init` | Инициализация `.arhit/`, автоопределение языка/фреймворка, создание `CLAUDE.md` |
...
При выполнении `arhit init` в файл `CLAUDE.md` (или `.claude/CLAUDE.md`, если существует) добавляется секция с инструкциями для ИИ-агента:
